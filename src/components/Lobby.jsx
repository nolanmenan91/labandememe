...
import { getLobbyPlayers, updatePlayerReady, updateLobbyStatus, leaveLobby, updateLobbySettings, updateLobbyRound, joinLobby } from '../services/db'
...
export default function Lobby({ lobby, onLeave, onGameStart, onLobbyUpdate, theme = 'default' }) {
  ...
  }, [players, loading, user.id, lobby.code])
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <RetroBox title="JOUEURS CONNECTÉS" theme={theme}>
        {loading ? (
      ...
      <div style={{ display: 'flex', gap: '15px', width: '100%' }}>
        <RetroButton onClick={handleLeave} theme={theme} style={{ flex: 1, backgroundColor: '#c21a1a', color: '#fff' }}>
          QUITTER
        </RetroButton>
        ...
        {!isHost ? (
          <RetroButton
            ...
            theme={theme}
            style={{ flex: 2, backgroundColor: myPlayer?.is_ready ? 'var(--code-bg)' : 'var(--accent-bg)', color: myPlayer?.is_ready ? 'var(--text)' : 'var(--accent)' }}
          >
